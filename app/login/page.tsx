...
import { Laptop, Mail, Lock, AlertCircle, ArrowRight } from "lucide-react";
  ...
  return (
    <div className="max-w-md w-full space-y-8 bg-slate-900/40 border border-slate-800 p-8 rounded-2xl shadow-xl backdrop-blur-md relative z-10">
      ...
      <div className="text-center">
        ...
        <h2 className="mt-6 text-xl font-bold text-white tracking-tight">Đăng nhập tài khoản</h2>
        <p className="mt-1.5 text-xs text-slate-500">Trải nghiệm mua sắm laptop chuyên nghiệp</p>
      </div>
      ...
      <form className="mt-8 space-y-4" onSubmit={handleSubmit}>
        ...
        <div className="space-y-1">
          <div className="flex justify-between items-center">
            ...
            <Link href="#" className="text-[10px] text-indigo-400 hover:underline">
              Quên mật khẩu?
            </Link>
          </div>
          <div className="relative">
        ...
        >
          {loading ? "Đang xử lý..." : "Đăng Nhập"} <ArrowRight className="h-4 w-4" />
        </button>
      </form>
      ...
      <div className="relative my-6 flex items-center justify-center">
        ...
        <span className="absolute bg-slate-900 px-3 text-[10px] text-slate-500 uppercase">Hoặc đăng nhập bằng</span>
      </div>

      {/* Google Login button */}
      <button
        onClick={handleGoogleLogin}
        className="w-full bg-slate-950 hover:bg-slate-800 border border-slate-800 text-slate-300 font-bold py-2 rounded-lg text-xs transition flex items-center justify-center gap-2"
      >
...
export default function LoginPage() {
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-950 px-4 py-12 relative overflow-hidden">
      ...
      <Suspense fallback={
        <div className="text-center py-20 text-slate-400 text-xs">Đang tải form đăng nhập...</div>
      }>
        <LoginForm />
      </Suspense>
    </div>
